...
export abstract class BaseToolHandler {
  protected runToolWithAPIKey(args: any): Promise<CallToolResult> {
    const oauth2Client = new OAuth2Client();
    oauth2Client.setCredentials({ access_token: args.access_token });
    log(
      `[${new Date().toISOString()}] runToolWithAPIKey. Access Token: ${
        args.access_token
      }\n`
    );
    ...
    if (error instanceof GaxiosError) {
      ...
      if (status === 403) {
        throw new McpError(
          ErrorCode.InvalidRequest,
          `Access denied: ${
            errorData?.error?.message || "Insufficient permissions"
          }`
        );
      }
